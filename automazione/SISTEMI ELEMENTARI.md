...
## RISPOSTA A INGRESSO SINUSOIDALE

## SVILUPPO IN SERIE DI FOURIER
Ogni segnale periodico può essere espresso come **combinazione lineare** di segnali cosinusoidali mediante lo **sviluppo in serie di Fourier**

Cioè una funzione periodica f(t) con perioto T può essere rappresentata mediante la **serie di Fourier**:
$$
f(t)=r_{0}+\sum^\infty_{n=1}r_{n}\cos(n\omega_{0}t + \phi_{n})
$$
dove:
$$
\omega_{0}=\frac{2\pi}{T}
$$
- $r_{0}$ nel caso sia presente si chiama **componente continua**
- $r_{n}\cos(n\omega_{0}t + \phi_{n})$ è datte **n-esima armonica**
- $r_{n} \geq 0$ è l'**ampiezza** della n-esima armonica
- $\phi$ è l'angolo che specifica lo **sfasamento** della n-esima armonica

Esistono segnali dove lo sviluppo in serie è composto da un numero finito di termini, oppure dove l'ampiezza di un segnale risulta trascurabile.
per queste bisgona definire una **bande del segnale**, un intervallo di pulsazioni $[n_{L},\omega_{0},n_{U},\omega_{0}]$ ovvera le pulsazioni associate ad armoniche con ampiezza significativa.

# TRASFORMATA DI FOURIER
Quando una funzione non è periodica non è possibile effettuare la serie di Fourier.
In questo caso bisogna introdurre la **trasfromata di Fourier**, che se soddisfatte certe condizioni è possobile eseguire questa operazione.

La trasformata di Fourier viene calcolata come:
$$
\mathcal F[f(t)]=F(j\omega) = \int^\infty_{-\infty} f(t)e^{-j\omega t}dt
$$

La trasformata di Fourier presenta molte similitudini con la [[SISTEMI E MODELLI#TRASFORMATA DI LAPLACE MODELLO IU|Trasformata di Laplace]], dove nel nostro caso $f(t)=0,t<0$, per questo $F(\omega)=F(s)|s=j\omega$.

La funzione $F(\omega)$ viene detta anche **spettro di f(t)**:
- **spettro di Ampiezza** $\to|F(\omega)|$
- **spettro di Fase** $\to arg \{ F(\omega)\}$ 

È possibile calcolare f(t) partendo dalla sua trasformata, essendo che la trasformata è una **funzione biunivoca**.

## ANTITRASFORMATA
Si ha:
$$
f(t)=\frac{1}{2\pi} \int^\infty_{0} |F(j\omega)|\cos(\omega t + argF(j\omega))d\omega
$$
Da qui si può capire che una **funzione non periodica** può essere scomposta in una **somma di segnali cosinusoidali, dette armoniche**.
[[AuIn06.1 - Analisi Armonica.pdf#page=20|AuIn06.1 - Analisi Armonica, pagina 20]]


# Rappresentazione di una funzione armonica
## DIAGRAMMI DI BODE
